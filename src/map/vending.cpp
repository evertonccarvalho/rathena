...
#include <common/utils.hpp> // External vending by [Snake]
...
**/
	if (battle_config.extended_vending) {
		if (vsd->vend_loot == battle_config.item_zeny || !vsd->vend_loot) {
			pc_payzeny(sd, (int32)z, LOG_TYPE_VENDING, sd->status.char_id);
			...
			pc_getcash(vsd, (int32)z, 0, LOG_TYPE_VENDING);
			...
			for (i = 0; i < MAX_INVENTORY; i++)
				if (sd->inventory.u.items_inventory[i].nameid == vsd->vend_loot){
					...
					pc_additem(vsd, item, (int32)z, LOG_TYPE_VENDING);
				}
			pc_delitem(sd, pc_search_inventory(sd, vsd->vend_loot), (int32)z, 0, 6, LOG_TYPE_VENDING);
		}
	} else {
		pc_payzeny(sd, (int32)z, LOG_TYPE_VENDING, sd->status.char_id);
		achievement_update_objective(sd, AG_SPEND_ZENY, 1, (int32)z);
	...
	if (battle_config.feature_autotrade) {
		...
		if( Sql_NumRows(mmysql_handle) > 0 ) {
			...
			while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
				...
				Sql_GetData(mmysql_handle, 8, &data, nullptr); at->vend_loot = atoi(data);		// Extended Vending system [Lilith]
																								// Att: 22/12/2024 By [KWDev]
				at->count = 0;
